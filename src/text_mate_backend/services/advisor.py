...
class AdvisorService:
    ...
    def get_docs(self, user: User) -> list[RuelDocumentDescription]:
        """
        Returns documentation file names available for advisor service.
        """
    ...
    async def check_text(self, text: str, docs: set[str]) -> RulesValidationContainer:
        """
        Checks text for any violations of rules and returns an aggregated result.
        """
    ...
    async def check_text_stream(self, text: str, docs: set[str]) -> AsyncIterator[RulesValidationContainer]:
        """
        Checks text for any violations of rules and yields validation results
    ...
    async def _check_text_stream(self, text: str, docs: set[str]) -> AsyncIterator[RulesValidationContainer]:
        ...
        if not rules:
            # Maintain parity with non-streaming API by yielding a single empty container
    ...
    async def _run_rule_validation(self, formatted_rules: str, text: str) -> RulesValidationContainer:
        """
        Validate input text against the provided, formatted editorial rules using the configured LLM.
        ...
        """
        prompt = f"""You are an expert in editorial guidelines. Review only of given rules and
                ...
                {str(RulesValidationContainer.model_json_schema())}
        ...
        return await self.llm_facade.structured_predict(
